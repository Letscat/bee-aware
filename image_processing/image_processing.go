	...
	for i := range image {
		for j := range image[i] {
			if image[i][j] == 255 {
				...
				if len(contour) > n {
					return true // Return the big contour
				}
			}
		}
	}
...
func dfs(image [][]uint8, x, y int, contour *[]int) {
	// Check if the pixel is out of bounds or not a contour pixel
	if x < 0 || x >= len(image) || y < 0 || y >= len(image[x]) || image[x][y] != 255 {
		return
	}

	// Mark the pixel as visited by setting its value to 0
	image[x][y] = 0

	// Add the pixel to the contour slice
	*contour = append(*contour, x, y)

	// Recursively search for neighboring pixels
